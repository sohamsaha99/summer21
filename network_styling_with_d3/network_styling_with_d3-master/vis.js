...
function vis(new_controls) {
  ...
  var isWeb = window.location['href'].includes("file:///");
  ...
  function canvas_arrow(context, fromx, fromy, tox, toy, r, linkText){
	  var x_center = (2*tox+fromx)/3;
	  var y_center = (2*toy+fromy)/3;
	  ...
	  context.fill();
	  context.font = clip(r, 10, 20) + "px Helvetica"
    context.fillStyle = controls['node_label_color']
    context.fillText(linkText, (tox+fromx)/2, (toy+fromy)/2)
  }
  ...
  function drawLink(d) {
    ...
    context.stroke();
    canvas_arrow(context, zoomScaler(d.source.x), zoomScaler(d.source.y), zoomScaler(d.target.x), zoomScaler(d.target.y), context.lineWidth, valIfValid(d.type, ""));
    // console.log(d)
  }
