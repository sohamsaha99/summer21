...
float p_transmission=0.022; ////////////////////////////////////////////////////////////////////////////////////////////
float low_risk_adjustment=0.89;////////////////////////////////////////////////////////////////////////////////////////
...
public:
    ...
    void statechange(vector<vector<pair<int, char>>> &eventq, char newstate, int currtime, unordered_set<int> &students_with_apps, float param_p_traced=p_traced, float param_p_tested=p_tested, int param_tracelength=tracelength_d, int param_quarantine_length=quarantine_length/timestep_in_data, int param_manual_tracing_threshold=manual_tracing_threshold, int param_app_tracing_threshold=app_tracing_threshold, float param_p_test_high_contact=p_test_high_contact, float param_p_test_low_contact=p_test_low_contact)//, set<int> students_with_apps)// SHOULDSETPARAMS
    {
        ...
        else if(newstate=='G' && !awaiting_result && !testedPositive)
        {
        ...
        else
        {
            ...
            if(state != 'J' && !awaiting_result && !testedPositive && (state=='M' || static_cast <float>(rand())/static_cast <float>(RAND_MAX) < param_p_tested))
            {
