...
Original file is located at
    https://colab.research.google.com/drive/1hTooEn1Rsb7juT22gZtjUTapai3EOLtS
"""
...
class Node:
    ...
    def __init__(self,params,myid=0,currtime=0):
        ...
        self.contacts={}            # traced contacts (dict[student_id]=[time_1,time_2])
...
def read_cluster(filename_root,filename_upto,datapath='your_path_here',normalizer=692.0):
    ...
    for i in range(0,filename_upto+1):
...
def plot_pandemic(output):
    """
    Plots the pandemic based on output. 'Output' parameter is a dictionary, with keys=Timestamp(in seconds)
    and values=Dictionary{'S': , 'E': , 'I': , 'R': }. Plots those 4 quantities using plotly
    """
    ...
    if not bool(temp):
        ...
        temp = output[times[0]]
    # print(temp)
    N_people = temp['S'] + temp['E'] + temp['I'] + temp['R']
    S = []
    E = []
    I = []
    R = []
...
def update_plot_output(output, time, N_people, exposed, infectious, total_infected):
    output[time]['S'] = N_people - total_infected
    output[time]['E'] = exposed
    output[time]['I'] = infectious
    output[time]['R'] = total_infected - infectious - exposed
...
if __name__=="__main__":
    ...
    # SEIR_onerun_grid(contactdict, student_ids)
...
if True:
    ...
    print("Day "+str('%.2f'%(curr_time/day))+" "+"First patient: P"+str(studentlist[patient_zero_index].id)+". E="+str(exposed)+", I=0, total_infected=1")
    done=False
    quarantines=0
    false_quarantines=0
    periodic_boundary_modifier=0
    while not (done):
        # --------- handle event queue
        # if curr_time >= day*days_passed and days_passed > curr_time//day:
        #     days_passed = curr_time//day
        #     print(days_passed)
        # print(curr_time)
        # break
        if curr_time in eventq:
            ...
            for event in eventq[curr_time]: # eventq events: (student_id,new_state)
                if event[1]=='BOQ_t' and not(studentlist[event[0]].in_quarantine):
                    quarantines+=1
                    print("Day "+str('%.2f'%(curr_time/day))+" P"+str(studentlist[event[0]].id)+" in quarantine. Total Q="+str(quarantines))
                    if studentlist[event[0]].state=='S' or studentlist[event[0]].state=='R':
                        false_quarantines+=1
                        print("NOTE: False quarantine. FQ="+str(false_quarantines))
                elif event[1]=='R': # book-keeping for the various classes (S=susceptible,E=exposed,Ip=presymptomatic infectious,I=infectious, Ic=cumulative number of infected, R=recovered)
                    infectious-=1
                    update_plot_output(output_for_plot, curr_time, N_students, exposed, infectious, total_infected)
                    print("Day "+str('%.2f'%(curr_time/day))+" P"+str(studentlist[event[0]].id)+" recovered. I="+str(infectious))
                elif event[1][0]=='I' and event[1]=='Ip':
                    exposed-=1
                    infectious+=1
                    update_plot_output(output_for_plot, curr_time, N_students, exposed, infectious, total_infected)
                    print("Day "+str('%.2f'%(curr_time/day))+" P"+str(studentlist[event[0]].id)+" became "+event[1]+". E="+str(exposed)+", I="+str(infectious))
                elif event[1][0]=='I':
                    print("Day "+str('%.2f'%(curr_time/day))+" P"+str(studentlist[event[0]].id)+" became "+event[1]+". E="+str(exposed)+", I="+str(infectious))
                studentlist[event[0]].statechange(eventq,event[1],curr_time,params,students_with_apps) # change student's state (disease progression, quarantine)
            del eventq[curr_time]
        ...
        if (curr_time-periodic_boundary_modifier)>max_time:
            periodic_boundary_modifier+=max_time+300
            print("Day "+str('%.2f'%(curr_time/day))+" "+"periodic_boundary_modifier="+str(periodic_boundary_modifier))
        if (curr_time-periodic_boundary_modifier) in contactdict: # if there are contacts at time t=curr_time
            # print("Yes")
            for contact in contactdict[(curr_time-periodic_boundary_modifier)]: # loop through all contacts that take place at t=curr_time
                if not(studentlist[contact[0]].state=='S' and studentlist[contact[1]].state=='S') and not(studentlist[contact[0]].in_quarantine or studentlist[contact[1]].in_quarantine): # note: works only for this particular SEIR timeline (incubation time etc)
    ...
    # for ssdd in student_id_list:
    #     stud0 = studentlist[ssdd]
    #     print(stud0.contacts)
