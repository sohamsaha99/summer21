...
def check_estimated_source(arr: pd.Series):
    if (not math.isnan(arr["estimated_source"])) and np.abs(arr["source"] - arr["estimated_source"]) <= 2:
        return True
...
# %%
df = cases.assign(check_result = cases.apply(lambda x: check_estimated_source(x), axis=1))
